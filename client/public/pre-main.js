...
function getTempUsers() {
  return localStorage.getItem("tempUsers") != undefined
    ...
    .then(() => {
      ...
      channel
        ...
        .then(() => {
          ...
          console.log(
            "**********shiv*********channel joined successfully**********"
          );
          ...
          channel
            ...
            .then(() => {
              console.log(
                "-------join msg llllll--------",
                "mssages send successfully on channel"
              );
            })
            .catch(error => {
              displayError(error);
          ...
          channel.on("MemberJoined", memberId => {
            ...
            $("#online-user-row-" + convertEmailToId(memberId))
              .find(".user-status")
            ...
            if (userList != "") {
              for (let j in userList) {
                if (userList[j].id == memberId) {
                  $("#online-user-row-" + convertEmailToId(memberId)).attr(
                    "data-position",
                    userList[j].joinAt
                  );
                  break;
            ...
            function sort_li(a, b) {
              return parseInt($(b).attr("data-position")) <
                parseInt($(a).attr("data-position"))
                ? 1
                : -1;
            }
            $("#online-user-list tr")
              .sort(sort_li)
              .appendTo("#online-user-list");
            ...
            if (storeData.userType == 1) {
              if ($("#joinee-" + convertEmailToId(memberId)).length == 0) {
                removeFromFirst();
                $("#joiners").append(
                  '<span class="welcome-title" id="joinee-' +
                    convertEmailToId(memberId) +
                    '">' +
                    getUserDataFromList(memberId, "firstName") +
                    " " +
                    (getUserDataFromList(memberId, "lastName") != null
                      ? getUserDataFromList(memberId, "lastName")
                      : "") +
                    "</span>"
                );
                totalChannelMembers();
         ...
         */
          ...
          channel.on("MemberLeft", memberId => {
            ...
            if (storeData.userType == 1) {
              if ($("#joinee-" + convertEmailToId(memberId)).length != 0) {
                $("#joiners")
                  .find("#joinee-" + convertEmailToId(memberId))
                  .remove();
          ...
          channel.on("ChannelMessage", (message, senderId) => {
            ...
            if (storeData.userType == 1) {
              if ($("#joinee-" + convertEmailToId(senderId)).length == 0) {
                ...
                $("#joiners").append(
                  '<span class="welcome-title" id="joinee-' +
                    convertEmailToId(senderId) +
                    '">' +
                    getUserDataFromList(senderId, "firstName") +
                    " " +
                    (getUserDataFromList(senderId, "lastName") != null
                      ? getUserDataFromList(senderId, "lastName")
                      : "") +
                    "</span>"
                );
                totalChannelMembers();
        ...
        .catch(error => {
          displayError(error);
          console.log(
            "**********shiv*********There Is a problem to join a channel**********"
          );
        });
    })
    .catch(err => {
...
function onclickaudioOn(audienceID) {
  ...
  $("#subscribers-list #agora_remote" + audienceID)
    .find(".hand-icon")
    .addClass("d-none");
  $("#subscribers-list #agora_remote" + audienceID)
    .find(".microphone-icon")
    .addClass("d-none");
...
function networkBandwidth() {
  ...
  // initialize client
  localClient.init(
    storeData.sessionData.appId,
    function() {
      console.log("-------------------------------------------HHH");
      // create and join channel
      localClient.join(
        storeData.sessionData.streamToken,
        storeData.sessionData.channelId.toString(),
        storeData.id,
        function(uid) {
  ...
  setInterval(function() {
    localClient.getTransportStats(stats => {
      ...
      console.log(
        `Current Transport OutgoingAvailableBandwidth: ${stats.OutgoingAvailableBandwidth}`
      );
    });
  }, 3000);
}
    ...
    for (var i = 0, ctr = 0, ctr1 = 0; i !== devices.length; ++i) {
      ...
      if (device.kind === "audioinput") {
        ...
        if (microphoneId == null) {
          if (ctr1 == 0) defaultSetting = "checked";
        } else {
        ...
        adoMediaHtml =
          '<div class="" id="ado-' +
          deviceId +
          '"><input class="form-radio" type="radio" name="audio-type" id="lbl-' +
          deviceId +
          '" value="' +
          deviceId +
          '" ' +
        ...
        if (cameraId == null) {
          if (ctr == 0) defaultSetting = "checked";
        } else {
        ...
        vdoMediaHtml =
          '<div class="col-12 col-md-3 mx-auto" id="vdo-' +
          deviceId +
          '"><div id="local-media-' +
          deviceId +
          '" ></div><div class="check-camera"><input type="radio" class="form-radio" name="video-type" id="lbl-' +
          deviceId +
          '" value="' +
          deviceId +
          '" ' +
...
function removePreScreenSession() {
  if (stream1 != undefined && stream1 != null) stream1.close();
  if (stream2 != undefined && stream2 != null) stream2.close();
}
...
function onPageResize() {
  ...
  if (sub_list_x <= 992) {
    if (len_subs > 2) {
      $("#subscribers-list").addClass("display-grid-auto-2");
    }
  } else {
    if (len_subs > 4) {
      $("#subscribers-list").addClass("display-grid-auto-4");
    }
  }
  setTimeout(function() {
    ...
    $(
      ".newcss.one, .newcss.two, .newcss.three, .newcss.four, .newcss.five"
    ).height(`${newHt - 2}px`);
    $(
      ".newcss.one, .newcss.two, .newcss.three, .newcss.four, .newcss.five"
    ).width(`${newWt - 2}px`);
...
function showHideScript() {
  if ($(".add-remove-round").hasClass("top-rounded")) {
    setTimeout(function() {
      $(".add-remove-round")
        .addClass("rounded")
        .removeClass("top-rounded");
    }, 400);
  } else {
    $(".add-remove-round")
      .removeClass("rounded")
      .addClass("top-rounded");
...
function countDown() {
  ...
  activeEle
    .find(".count-box svg circle")
    .attr("style", "animation-duration:" + countdown + "s !important");
  ...
  if (userType == 1) {
    ...
    if (resultant[0] == "201") {
      ...
      let message =
        ...
        localDta.firstName +
        ", this is " +
        getUserDataFromList(uid, "firstName") +
        ", welcome to your first virtual session with us  ";
    ...
    if (resultant[0] == "204") {
      ...
      let message =
        ...
        ", this is " +
        getUserDataFromList(uid, "firstName") +
        ", welcome to your 1st virtual session with us  ";
...
function showHandAtHost() {
  ...
  for (let i in audienceList) {
    list +=
      '<li><a class="dropdown-item media" href="#"><img src="images/avtar.png" /><div  class="media-body"><span>' +
      audienceList[i].firstName +
      ", LA</span><span>2 min ago</span></div></a></li>";
    /*
...
function totalChannelMembers() {
  ...
  channel
    ...
    .then(membersList => {
      ...
      $("#total-joinees").html(
        totMember > maxUserLimit ? `+${maxUserLimit} more` : ""
      );
    })
    .catch(error => {
      console.log("*************There is an error******");
    ...
    .then(membersList => {
      ...
      for (let j = 0; j < strArray.length; j++) {
        for (let i = totMember - 1; i >= 0; i--) {
          if (
            membersList[i].id == strArray[j].id &&
            getUserDataFromList(membersList[i], "userType") == 2
          ) {
            if (ctr++ <= maxUserLimit) {
              if (
                $("#joinee-" + convertEmailToId(membersList[i])).length == 0
              ) {
                $("#joiners").append(
                  '<span class="welcome-title" id="joinee-' +
                    convertEmailToId(membersList[i]) +
                    '">' +
                    getUserDataFromList(membersList[i], "firstName") +
                    " " +
                    (getUserDataFromList(membersList[i], "lastName") != null
                      ? getUserDataFromList(membersList[i], "lastName")
                      : "") +
                    "</span>"
                );
              }
      ...
      $("#total-joinees").html(
        totMember - 1 > maxUserLimit ? `+${maxUserLimit} more=` : ""
      );
    })
    .catch(error => {
      console.log("*************There is an error******");
...
function getOrderUser() {
  ...
  userList.sort(function(a, b) {
    return parseInt(a.joinAt) - parseInt(b.joinAt);
  });
...
function getAllAudience() {
  ...
  if (tempUsers != null) {
    for (let i in tempUsers) {
      if (
        tempUsers[i].hasOwnProperty("isSubscribe") &&
        tempUsers[i].isSubscribe == 2
      ) {
        audience.push(tempUsers[i]);
      }
    }
    audience.sort(function(a, b) {
      return parseInt(a.subscribeTime) - parseInt(b.subscribeTime);
    });
  }
