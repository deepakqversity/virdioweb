...
class Session{
	...
	async getUpcommingSession(userId) {
        return await new Promise((resolve, reject) => {
        	db.query('SELECT s.*, i.code, su.sessionId, su.userId, su.type, ac.appId, ac.appCertificate, u.name as hostName FROM sessions s LEFT JOIN session_users su ON su.sessionId = s.id LEFT JOIN agora_config ac ON ac.id = s.configId JOIN users u ON u.id = s.hostId LEFT JOIN interest i ON i.id = s.interestId WHERE su.userId = ? AND su.status = 1 AND s.status = 1 AND s.scheduleDate IS NOT NULL AND s.scheduleDate > ( NOW() - INTERVAL 10 MINUTE ) ORDER BY s.scheduleDate ASC LIMIT 1', [userId], function (error, results, fields) {
